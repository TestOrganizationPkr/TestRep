...
describe('Redis-DB-Unit Test', function () {
    ...
    describe('Method Save', function () {
        ...
        it('should be able to save without problems', function (done) {
            ...
            return redisClient.rpush(REDIS_KEY, JSON.stringify(testData), function (error, result) {
    ...
    describe('Method Get All', function () {
        it('should be able to retrieve all records without any problem', function (done) {
            return redisClient.lrange(REDIS_KEY, 0, -1, function (error, result) {
                expect(error).to.be(null);
                //Since a previous insert has been done, there should be atleast one testData in the database
    ...
    describe('Method Update', function () {
        ...
        it('should be able to update without problems', function (done) {
            testData.name = 'Number two';
            // update using the index of the record
            return redisClient.lset(REDIS_KEY, index, JSON.stringify(testData), function (error, result) {
                expect(error).to.be(null);
    ...
    describe('Method Delete', function () {
        ...
        it('should be able to delete without problems', function (done) {
            ...
            return redisClient.lrem(REDIS_KEY, 0, JSON.stringify(testData), function (error, result) {
                expect(error).to.be(null);
    ...
    after(function (done) {
        // delete the input record
        redisClient.lrem(REDIS_KEY, 0, JSON.stringify(testData), function () {
            done();
        });
    });
});
