...
angular.module('toDo',[])
    .controller('ToDoController',  ['$scope','Todos', function($scope, Todos) {
		...
		$scope.canShowBg = false;

/*This is to hide popup start*/
$scope.hidepopup = function () {
	$scope.canShowBg = false;
	$scope.canShowSms = false;
	$scope.canShowEmail = false;
}
/*This is to hide popup end*/
		/*This is for sms start*/
$scope.canShowSms = false;
$scope.showpopupSms = function (body) {
	$scope.canShowBg = !$scope.canShowBg;
	$scope.canShowSms = !$scope.canShowSms;
	
	angular.element(toNumber).val("");
	angular.element(smsBody).val(body);
}

$scope.sendSms = function () {
	Todos.sendsms({ "toNumber": angular.element(toNumber).val(), "smsBody": angular.element(smsBody).val() })
			.success(function (data) {
				
			});
	$scope.hidepopup();
}
		/*This is for email start*/
  $scope.canShowEmail = false;
  $scope.showpopupEmail = function (body) {
  $scope.canShowBg = !$scope.canShowBg;
  $scope.canShowEmail = !$scope.canShowEmail;
  angular.element(toEmail).val("");
  angular.element(emailBody).val(body);
   }
    $scope.sendEmail = function () {
  Todos.sendEmail({ "toEmail": angular.element(toEmail).val(), "emailBody": angular.element(emailBody).val() })
  .success(function (data) {
	 });
      $scope.hidepopup();
    }		
